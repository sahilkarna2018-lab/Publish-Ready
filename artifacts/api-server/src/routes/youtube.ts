...
router.post("/youtube-multilingual", async (req, res) => {
  try {
    ...
    (req as any).log?.error ? (req as any).log.error({ err: e }, "YouTube multilingual proxy error") : console.error(e, "YouTube multilingual proxy error");
...
router.post("/youtube", async (req, res) => {
  try {
    ...
    (req as any).log?.error ? (req as any).log.error({ err: e }, "YouTube proxy error") : console.error(e, "YouTube proxy error");
    res.status(500).json({ error: "Failed to fetch YouTube videos" });
  }
});
